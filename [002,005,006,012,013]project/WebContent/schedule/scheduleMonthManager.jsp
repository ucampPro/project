...
<script type='text/javascript'>
	...
	var data="[{'title' : '${list[0].firstTeam} VS ${list[0].secondTeam}','start' : new Date(${list[0].year}, ${list[0].month-1}, ${list[0].day}), 'url' : 'sController?action=selectOne&code=${list[0].code}'}";
	for(i=1; i<${size}; i++){
		data += ",{'title' : '"+firstTeams[i]+" VS "+secondTeams[i]+"','start' : new Date("+years[i]+", "+parseInt(months[i]-1)+", "+days[i]+"), 'url' : 'sController?action=selectOne&code="+codes[i].trim()+"'}";
	}
	data += "]";
	data = eval('(' + data + ')');
